...
RColor RColor::FromREC709() const
{
	XMVECTOR T = XMVectorSaturate(Value);
...
uint32_t RColor::R10G10B10A2() const
{
...
uint32_t RColor::R8G8B8A8() const
{
...
uint32_t RColor::R11G11B10F(bool RoundToEven /*= false*/) const
{
#if 1
	...
	R.f = RMath::Clamp(Value.f[0], 0.0f, kMaxVal) * kF32toF16;
	G.f = RMath::Clamp(Value.f[1], 0.0f, kMaxVal) * kF32toF16;
	B.f = RMath::Clamp(Value.f[2], 0.0f, kMaxVal) * kF32toF16;
...
uint32_t RColor::R9G9B9E5() const
{
#if 1
	...
	float r = RMath::Clamp(Value.f[0], 0.0f, kMaxVal);
	float g = RMath::Clamp(Value.f[1], 0.0f, kMaxVal);
	float b = RMath::Clamp(Value.f[2], 0.0f, kMaxVal);

	// Compute the maximum channel, no less than 1.0*2^-15
	float MaxChannel = RMath::Max(RMath::Max(r, g), RMath::Max(b, kMinVal));
