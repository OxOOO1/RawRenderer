...
#include <fstream>
#include <cassert>
#include "Core/Renderer/CommandContext.h"
#include "Core/Renderer/LLRenderer.h"

void RPixelBuffer::ExportToFile(const std::wstring& FilePath)
{
	// Create the buffer.  We will release it after all is done.
	RReadbackBuffer TempBuffer;
	TempBuffer.Create(L"Temporary Readback Buffer", Width * Height, (uint32_t)BytesPerPixel(Format));

	RCommandList::ReadbackTexture2DImmediate(TempBuffer, *this);
	...
	OutFile.write((const char*)&Format, 4);
	OutFile.write((const char*)&Width, 4); // Pitch
...
void RPixelBuffer::AssociateWithResource(const std::wstring& Name, ID3D12Resource* inResource, D3D12_RESOURCE_STATES CurrentState)
{
	...
	Width = (uint32_t)ResourceDesc.Width;        // We don't care about large virtual textures yet
...
void RPixelBuffer::CreateTextureResource(const std::wstring& Name, const D3D12_RESOURCE_DESC& ResourceDesc, D3D12_CLEAR_VALUE ClearValue, D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr /*= D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN*/)
{
	Destroy();
...
DXGI_FORMAT RPixelBuffer::GetBaseFormat(DXGI_FORMAT Format)
{
	switch (Format)
	{
...
DXGI_FORMAT RPixelBuffer::GetUAVFormat(DXGI_FORMAT Format)
{
	switch (Format)
	{
...
size_t RPixelBuffer::BytesPerPixel(DXGI_FORMAT Format)
{
	switch (Format)
	{
